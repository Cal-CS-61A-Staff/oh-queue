...
import requests
from flask import render_template, url_for, copy_current_request_context, session
from flask_login import current_user
from flask_socketio import emit, join_room, leave_room
from sqlalchemy import func, desc

from oh_queue import app, db, socketio
from oh_queue.course_config import (
    get_course,
    format_coursecode,
    get_course_id,
    COURSE_DOMAINS,
...
def emit_event(ticket, event_type):
    ...
    socketio.emit(
        "event",
        {"type": event_type.name, "ticket": ticket_json(ticket)},
        room=get_course(),
    )
...
def connect():
    ...
    emit_state(
        ["tickets", "assignments", "locations", "current_user", "config"],
...
def assign_appointment(data):
    ...
    if not current_user.is_staff and not old_signup:
        check = validate_appointment(appointment, current_user)
        if check:
            return check
    ...
    assignment = Assignment.query.filter_by(id=data["assignment_id"]).one()
    group = list_members(assignment)
    for member in selected_group:
        if member not in group:
            return socket_error(
                "A selected student is not your group, or your OKPy token has expired."
            )

    group_name = "{}'s group".format(current_user.name)
    ...
    for member in selected_group:
        member_user = User.query.filter_by(email=member).one_or_none()
        if not member_user:
            member_user = User(
                name=member, email=member, is_staff=False, course=get_course()
            )
            db.session.add(member_user)
            db.session.commit()

        old_signup = AppointmentSignup.query.filter_by(
            appointment_id=data["appointment_id"], user_id=member_user.id, course=get_course()
        ).one_or_none()
        ...
        if old_signup:
            ...
            check = validate_appointment(appointment, member_user)
            if check:
                return check
        ...
        signups.append(
            AppointmentSignup(
                ...
                assignment_id=data["assignment_id"],
                question=data["question"],
                description="{}'s teammate".format(current_user.name),
...
def validate_appointment(appointment, user):
    daily_threshold = int(
        ConfigEntry.query.filter_by(key="daily_appointment_limit", course=get_course())
        .one()
        .value
    )
    weekly_threshold = int(
        ConfigEntry.query.filter_by(key="weekly_appointment_limit", course=get_course())
        .one()
        .value
    )
    pending_threshold = int(
        ConfigEntry.query.filter_by(key="simul_appointment_limit", course=get_course())
        .one()
        .value
    )
    ...
    num_this_week = (
        ...
        .filter(
            ...
            AppointmentSignup.user_id == user.id,
    ...
    if num_this_week >= weekly_threshold:
        return socket_error(
            "You have (or your teammate has) already signed up for {} OH slots this week".format(
                weekly_threshold
            )
        )
    ...
    if num_today >= daily_threshold:
        return socket_error(
            "You have (or your teammate has) already signed up for {} OH slots for the same day".format(
                daily_threshold
            )
        )
    ...
    if num_pending >= pending_threshold:
        return socket_error(
            "You have (or your teammate has) already signed up for {} OH slots that have not yet occurred.".format(
                pending_threshold
            )
        )
...
def upload_appointments(data):
    ...
    try:
        ...
        for row in data[1:]:
            ...
            group_only = (
                row[header.index("Only Groups")] == "True"
                if "Only Groups" in header
                else False
            )
...
def find_group(data):
    return list_members(Assignment.query.filter_by(id=data["assignment"]).one())


def list_members(assignment):
    assignment = assignment.name

    assignments = requests.get(
        "{server}/api/v3/course/{endpoint}/assignments".format(
            server=app.config.get("OK_SERVER_URL"), endpoint=get_endpoint()
        )
    ).json()
    assignments = {
        assignment["display_name"]: assignment["name"]
        for assignment in assignments["data"]["assignments"]
    }
    if app.config["ENV"] != "prod":
        assignments = {
            "Hog": "cal/cs61a/sp20/proj01",
            "Scheme": "cal/cs61a/sp20/proj04",
        }
    if assignment not in assignments:
        return []
    assignment = assignments[assignment]
    resp = requests.get(
        "{server}/api/v3/assignment/{assignment}/group/{email}".format(
            server=app.config.get("OK_SERVER_URL"),
            assignment=assignment,
            email=current_user.email,
        ),
        params={"access_token": session["access_token"][0]},
    ).json()
    if app.config["ENV"] != "prod":
        if assignment.endswith("proj01"):
            return ["steve@berkeley.edu"]
    ...
    return [member for member in members if member != current_user.email]
